<template>
    <div class="container mx-auto px-4 py-2">
        ...
        <div v-else class="space-y-4">
            <div class="overflow-hidden rounded-lg border border-gray-200 shadow-sm dark:border-gray-700 dark:shadow-none">
                <Table class="w-full">
                    ...
                    <TableBody class="divide-y divide-gray-200 dark:divide-gray-700">
                        <TableRow
                            v-for="almacen in almacenList"
                            :key="almacen.id"
                            class="transition-colors hover:bg-gray-50 dark:hover:bg-gray-800/30"
                        >
                            <TableCell class="px-4 py-3 font-medium text-gray-900 dark:text-gray-100">{{ almacen.id }}</TableCell>
                            <TableCell class="px-4 py-3 text-gray-700 dark:text-gray-300">{{ almacen.name }}</TableCell>
                            <TableCell class="px-4 py-3">
                                <span
                                    v-if="almacen.state === true"
                            ...
                            <TableCell class="px-4 py-3 text-gray-500 dark:text-gray-400">{{ almacen.created_at }}</TableCell>
                            <TableCell class="flex justify-start space-x-2 px-4 py-3">
                                <Button
                                    ...
                                    @click="openModal(almacen.id)"
                                    title="Editar almacen"
                                >
                                    <UserPen class="h-4 w-4" />
                                    <span class="sr-only">Editar almacen</span>
                                </Button>
                                <Button
                                    variant="ghost"
                                    size="sm"
                                    ...
                                    @click="openModalDelete(almacen.id)"
                                    title="Eliminar almacen"
                                >
                                    <Trash class="h-4 w-4" />
                                    <span class="sr-only">Eliminar almacen</span>
                                </Button>
                            </TableCell>
                        </TableRow>
                    </TableBody>
            ...
            <PaginationAlmacen :meta="almacenPaginate" @page-change="$emit('page-change', $event)" class="mt-6" />
        </div>
    </div>
</template>
...
import PaginationAlmacen from '../../../../components/pagination.vue';
import { AlmacenResource } from '../interface/Almacen';

const { toast } = useToast();

const emit = defineEmits<{
    (e: 'page-change', page: number): void;
    (e: 'open-modal', id_almacen: number): void;
    (e: 'open-modal-delete', id_almacen: number): void;
}>();
...
const { almacenList, almacenPaginate } = defineProps<{
    almacenList: AlmacenResource[];
    almacenPaginate: Pagination;
    loading: boolean;
}>();
