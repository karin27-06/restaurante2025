<template>
    <Head title="Nuevo Almacen"></Head>
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
            <Card class="mt-4 flex flex-col gap-4">
                <CardHeader>
                    <CardTitle>NUEVO ALMACEN</CardTitle>
                    <CardDescription>Complete los campos para crear un nuevo almacen</CardDescription>
                </CardHeader>
                <CardContent>
                    <form @submit="onSubmit" class="flex flex-col gap-6">
                        <!-- Campo para ingresar el nombre del Almacen -->
                        <FormField v-slot="{ componentField }" name="name">
                            <FormItem>
                                <FormLabel>Almacen</FormLabel>
                                <FormControl>
                                    <Input type="text" placeholder="Almacen" v-bind="componentField" />
                                </FormControl>
                                <FormMessage />
                            </FormItem>
                        </FormField>
...
//Importaciones para hacer el formulario de tipos de Almacen, ok mucho texto
...
const { createAlmacen } = useAlmacen();
